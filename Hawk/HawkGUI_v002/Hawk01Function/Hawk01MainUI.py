...
class Hawk01MainUI:
    ...
    def setup_script_gui(self):
        ...
        self.ui.load_pages.MIPI_RATE_ComboBox.add_items(self.hawk01_gui_config["MIPI_RATE"]["show_gui"])
        ...
        MIPI_RATE_indexs = [self.hawk01_gui_config["MIPI_RATE"]["config"].index(config) for config in
                            self.hawk01_config['MIPI_RATE']]
    ...
    def UPDATE_REF_CLK(self, i):
        ...
        logging.info(self.hawk01_config['REF_CLK'])
        return
    ...
    def UPDATE_WORK_MODE(self, i):
        ...
        logging.info(self.hawk01_config['WORK_MODE'])

    def UPDATE_SCAN_MODE(self, i):
        self.hawk01_config['SCAN_MODE'] = i
        logging.info(self.hawk01_config['SCAN_MODE'])

    def UPDATE_MST_MODE(self, i):
        self.hawk01_config['MST_MODE'] = i
        logging.info(self.hawk01_config['MST_MODE'])

    def UPDATE_TRG_I_EN(self, i):
        self.hawk01_config['TRG_I_EN'] = i
        logging.info(self.hawk01_config['TRG_I_EN'])

    def UPDATE_MIPI_RATE(self, i):
        select_indexs = self.ui.load_pages.MIPI_RATE_ComboBox.get_selected_index()
        self.hawk01_config['MIPI_RATE'] = list(
            map(lambda i: self.hawk01_gui_config["MIPI_RATE"]["config"][i], select_indexs))
        logging.info(self.hawk01_config['MIPI_RATE'])
    ...
    def UPDATE_TDC_BIN_W(self, i):
        ...
        self.ui.load_pages.SYS_CLK_ComboBox.setCurrentIndex(SYS_CLK_index)
        logging.info(f"{self.hawk01_config['TDC_BIN_W']}, {self.hawk01_config['SYS_CLK']}")

    def UPDATE_V_ROLL_NUM(self, num):
        self.hawk01_config['V_ROLL_NUM'] = num - 1
        logging.info(self.hawk01_config['V_ROLL_NUM'])

    def UPDATE_H_ROLL_NUM(self, num):
        self.hawk01_config['H_ROLL_NUM'] = num - 1
        logging.info(self.hawk01_config['H_ROLL_NUM'])

    def UPDATE_H_VLD_SEG(self, num):
        self.hawk01_config['H_VLD_SEG'] = num - 1
        logging.info(self.hawk01_config['H_VLD_SEG'])
    ...
    def func_roi_load_file(self):
        ...
        # logging.info(self.hawk01_config['cali_filename'])
        return
    ...
    def func_get_roi_config(self):
        ...
        if self.roi_gen_type == 0:   # Gen ROI for GUI
            ...
            # logging.info(self.hawk01_roi_gen_config['ROIGenByJson'])
            ...
            # logging.info(self.hawk01_roi_gen_config['ROIGenByFile'])
            ...
            # logging.info(self.hawk01_roi_gen_config['ROIGenByBase'])
        ...
        # elif self.roi_gen_type == 3: # Gen ROI for cali data
            ...
            self.hawk01_roi_gen_config['ROIGenByCali']['is_reverse'] = self.ui.load_pages.cali_order_ComboBox.currentIndex()
            self.hawk01_roi_gen_config['ROIGenByCali']['img_reverse'] = self.ui.load_pages.img_mirror_ComboBox.currentIndex()
    ...
    def func_merge_hawk_config(self):
        ...
        self.__hawk01_config__ = \
            {**self.hawk01_config, **self.hawk01_roi_gen_config['ROIGenByJson'], **self.hawk01_zone_config} if self.roi_gen_type == 0 else \
            {**self.hawk01_config, **self.hawk01_roi_gen_config['ROIGenByFile'], **self.hawk01_zone_config} if self.roi_gen_type == 1 else \
            {**self.hawk01_config, **self.hawk01_roi_gen_config['ROIGenByBase'], **self.hawk01_zone_config} if self.roi_gen_type == 2 else \
    ...
    def func_open_roizone_config_win(self, url):    # TODO:hawk01_zone_config需要传值
    ...
    def setup_file_gui(self):
        """ FILE config 界面GUI配置"""
        self.ui.load_pages.base_script_LineEdit.setText(self.hawk01_config['ref_cfg_file'])
        self.ui.load_pages.file_save_dir_LineEdit.setText(self.hawk01_config['fd_path'])
        self.ui.load_pages.REG_CFG_File_LineEdit.setText(self.hawk01_config['reg_name'])
        self.ui.load_pages.ROI_SRAM_File_LineEdit.setText(self.hawk01_config['roi_name'])
        self.ui.load_pages.ROI_Sram_File_CheckBox.setChecked(self.hawk01_config['ROI_SRAM_Include'])
        self.ui.load_pages.SPADISS_Integration_CheckBox.setChecked(self.hawk01_config['SPADISS_Integration'])

        self.ui.load_pages.ROI_Sram_File_CheckBox.stateChanged.connect(partial(Hawk01MainUI.func_file_gui_checkBoxChange, self))
        ...
        # 按钮绑定
        self.ui.load_pages.base_script_Button.clicked.connect(partial(Hawk01MainUI.func_base_script_file_sel, self))
        self.ui.load_pages.file_save_dir_Button.clicked.connect(partial(Hawk01MainUI.func_file_save_dir_sel, self))
        self.ui.load_pages.Save.clicked.connect(partial(Hawk01MainUI.func_get_file_name_config, self))
        return

    def func_file_gui_checkBoxChange(self, state):
        self.hawk01_config['ROI_SRAM_Include'] = self.ui.load_pages.ROI_Sram_File_CheckBox.isChecked()
        self.hawk01_config['SPADISS_Integration'] = self.ui.load_pages.SPADISS_Integration_CheckBox.isChecked()
        return

    def func_base_script_file_sel(self):
    ...
    def func_get_file_name_config(self):
        # self.hawk01_config['reg_name'] = self.ui.load_pages.REG_CFG_File_LineEdit.text()
        # self.hawk01_config['roi_name'] = self.ui.load_pages.ROI_SRAM_File_LineEdit.text()
        # logging.info(f"{self.hawk01_config['reg_name']}, {self.hawk01_config['roi_name']}")
