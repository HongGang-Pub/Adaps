...
def MipiPKGIntvCal(mipi_cfg: dict, SYS_CLK=330, MIPI_RATE=1500, TxEscClkDiv=None, non_continuous: bool = False):
    """
...
def MIPI_CONFIG_Cal(SYS_CLK=330, MIPI_RATE=1500, display=True):
    ...
    mipi_cfg = {
        ...
        "ClkTxThsprepareCnt": 0x00,         # TXClkEsc: config+1
    ...
    T_default_all_accu   = T_default_hs_lpx + T_default_hs_exit + T_default_hs_zero + T_default_hs_trail
    ...
    # print("T_clk_lpx:", T_clk_lpx)
    # print("T_clk_prepare:", T_clk_prepare)
    # print("T_clk_zero:", T_clk_zero)
    # print("T_clk_post:", T_clk_post)
    # print("T_clk_trail:", T_clk_trail)
    # print("T_clk_exit:", T_clk_exit)
    ...
    MIPI_PKT_INTV0 = MipiPKGIntvCal(mipi_cfg, SYS_CLK, MIPI_RATE, non_continuous=False)
    MIPI_PKT_INTV1 = MipiPKGIntvCal(mipi_cfg, SYS_CLK, MIPI_RATE, non_continuous=True)
    s = ""
    ...
    s += f"\n\tT_lpx               : {2:4}   | {T_hs_lpx:6.2f} | {T_std_lp_01:6.2f} | {T_default_hs_lpx:6.2f}"
    ...
    s += f"\n\tT_all_aacu          : {'':5}  | {T_all_accu:6.2f} | {T_std_all_accu:6.2f} | {T_default_all_accu:6.2f}"
    s += f"\n\tTime saving @default: {T_default_all_accu - T_all_accu:6.2f} ns"
...
if __name__ == "__main__":
    mipi_config = MIPI_CONFIG_Cal(SYS_CLK=330, MIPI_RATE=1000)
