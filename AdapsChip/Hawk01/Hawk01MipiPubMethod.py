...
def BinNumberAdd(pkg_data, bin_number=8):
    ...
    ps: 此方法仅适用 PCM & FHR, PHR & SPHR bin 数据不是从 index=0 开始
...
def GetSpecificMipiFile(f_dict, v_roll_num=None, h_roll_num=None):
    """
    获取指定条件的 MIPI 文件
    Args:
    ...
    Returns:
        符合筛选条件的 vroll_num, hroll_num, f_index
    """

    # mode: 0: vroll & hroll都相等的；1: vroll相等的；2: hroll相等的；3: 不检查，直接返回
    mode = 0 if v_roll_num is not None and h_roll_num is not None \
        else 1 if v_roll_num is not None and h_roll_num is None \
        else 2 if v_roll_num is None and h_roll_num is not None \
        else 3
    ...
    for f_idx in file_index_list:
        ...
        frame_id, vroll_num, hroll_num = GerMipiFrameInfo(file, one_dt_mode=0)
        if mode == 0:
